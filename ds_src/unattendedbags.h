...
enum PGIE_CLASS {baggage = 0, person = 0};

enum GIE_UID {BAG_DETECTOR = 1 , PERSON_DETECTOR = 2};

int num_sources = 0 , x3 =0 , x4 = 0 , y3 =0 , y4 =0 , x5 =0 , x6 = 0 , y5 =0 , y6 =0 ;
int distances = 0, k = 0,l =0 , temp = 0;
vector<int> bags_x , person_x ; 

namespace UnattendedBagDetection {
  class Forsaken {
    private:
      gchar pgie_bag_classes_str[1][10] = {
          "baggage"
        };
      gchar pgie_person_classes_str[1][10] = {
          "person"
        };
      ...
      inline static char *BAG_DETECTOR_CONFIG_FILE_PATH;
      inline static char *PERSON_DETECTOR_CONFIG_FILE_PATH;
    ...
    public:
      ...
      std::string BAG_ENGINE_PATH;
      std::string PERSON_ENGINE_PATH;
      static void
      update_fps (gint id);
      ...
      static void
      setObjColPost(NvDsFrameMeta* frame_meta, std::vector<int> is_closed_matrix);
      ...
      int
      configure_element_properties(int num_sources, GstElement *streammux, GstElement *person_detector, GstElement *bag_detector,
